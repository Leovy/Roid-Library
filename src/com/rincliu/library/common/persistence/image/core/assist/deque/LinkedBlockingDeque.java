...
package com.rincliu.library.common.persistence.image.core.assist.deque;
...
public class LinkedBlockingDeque<E> extends AbstractQueue<E> implements BlockingDeque<E>, java.io.Serializable
{
    ...
    /*
     * Implemented as a simple doubly-linked list protected by a single lock
     * and using conditions to manage blocking. To implement weakly consistent
     * iterators, it appears we need to keep all Nodes GC-reachable from a
     * predecessor dequeued Node. That would cause two problems: - allow a
     * rogue Iterator to cause unbounded memory retention - cause
     * cross-generational linking of old Nodes to new Nodes if a Node was
     * tenured while live, which generational GCs have a hard time dealing
     * with, causing repeated major collections. However, only non-deleted
     * Nodes need to be reachable from dequeued Nodes, and reachability does
     * not necessarily have to be of the kind understood by the GC. We use the
     * trick of linking a Node that has just been dequeued to itself. Such a
     * self-link implicitly means to jump to "first" (for next links) or
     * "last" (for prev links).
     */

    /*
     * We have "diamond" multiple interface/abstract class inheritance here,
     * and that introduces ambiguities. Often we want the BlockingDeque
     * javadoc combined with the AbstractQueue implementation, so a lot of
     * method specs are duplicated here.
     */
    ...
    static final class Node<E>
    {
        ...
        /**
         * One of: - the real successor Node - this Node, meaning the
         * successor is head - null, meaning there is no successor
         */
        Node<E> next;
     ...
     * this deque. This method differs from {@link #peek peek} only in that it
     ...
     * chance to interact with the collection, especially when count is close
     * to capacity.
     */
    ...
    /**
     * Atomically removes all of the elements from this deque. The deque will
    ...
    private abstract class AbstractItr implements Iterator<E>
    {
         ...
         * element exists in hasNext(), we must return item read under lock
         * (in advance()) even if it was in the process of being removed when
         * hasNext() was called.
         */
        E nextItem;

        /**
         * Node returned by most recent call to next. Needed by remove. Reset
         * to null if this element is deleted by a call to remove.
         */
