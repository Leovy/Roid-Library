 ...
 * have no capacity restrictions; they grow as necessary to support usage.
 * They are not thread-safe; in the absence of external synchronization, they
 * do not support concurrent access by multiple threads. Null elements are
 * prohibited. This class is likely to be faster than {@link Stack} when used
 * as a stack, and faster than {@link LinkedList} when used as a queue.
 * <p>
...
public class ArrayDeque<E> extends AbstractCollection<E> implements Deque<E>, Cloneable, Serializable {
     ...
     * around to equal each other. We also guarantee that all array cells not
     * holding deque elements are always null.
     */
    private transient E[] elements;

    /**
     * The index of the element at the head of the deque (which is the element
     * that would be removed by remove() or pop()); or an arbitrary number
    ...
    /**
     * The minimum capacity that we'll use for a newly created deque. Must be
     * a power of 2.
     */
     ...
     * element returned by the collection's iterator becomes the first
     ...
     * element <tt>e</tt> such that <tt>o.equals(e)</tt> (if such an element
     ...
     * be ordered from first (head) to last (tail). This is the same order
     * that elements would be dequeued (via successive calls to
    ...
    private class DeqIterator implements Iterator<E> {
        ...
        /**
         * Index of element returned by most recent call to next. Reset to -1
         * if element is deleted by a call to remove.
         */
        ...
        public void remove() {
            ...
            if (delete(lastRet)) { // if left-shifted, undo increment in
                                   // next()
                cursor = (cursor - 1) & (elements.length - 1);
                fence = tail;
            }
     ...
     * the specified element (or equivalently, if this deque changed as a
     * result of the call).
     * <p>
     ...
     * array is that of the specified array. If the deque fits in the
