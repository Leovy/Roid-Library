...
public class RLWeiboHelper {
	 ...
	 * @param handler
	 */
	public void sendText(final Context context, String content,
			String latitude, String longitude, final ReqHandler handler){
		new StatusesAPI(AccessTokenKeeper.readAccessToken(context)).update(content, 
				latitude, longitude,getRequestListener(context,handler));
	}
	 ...
	 * @param handler
	 */
	public void sendImageText(final Context context, String content, File imgFile,
			String latitude, String longitude, final ReqHandler handler){
		new StatusesAPI(AccessTokenKeeper.readAccessToken(context)).upload(content,
				imgFile.getPath(), latitude, longitude,getRequestListener(context,handler));
	}
	 ...
	 * @param handler
	 */
	public void sendImageText(final Context context, String content, URL imgUrl,
			String latitude, String longitude, final ReqHandler handler){
		new StatusesAPI(AccessTokenKeeper.readAccessToken(context)).uploadUrlText(content,
				imgUrl.toString(), latitude, longitude,getRequestListener(context,handler)); 
	}
