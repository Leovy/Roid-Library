 ...
 * https
 * ://android.googlesource.com/platform/frameworks/base/+/jb-release/core/
 * java/android/os/AsyncTask.java so that threading behavior on all OS
 * versions is the same and we can tweak behavior by using executeOnExecutor()
 * if needed. There are 3 changes in this copy of AsyncTask: -pre-HC a single
 * thread executor is used for serial operation
 * (Executors.newSingleThreadExecutor) and is the default -the default
 * THREAD_POOL_EXECUTOR was changed to use DiscardOldestPolicy -a new fixed
 * thread pool called DUAL_THREAD_EXECUTOR was added
 * *************************************
 * <p>
 ...
 * AsyncTasks should ideally be used for short operations (a few seconds at
 * the most.) If you need to keep threads running for long periods of time, it
 * is highly recommended you use the various APIs provided by the
 ...
 * task is executed. This step is normally used to setup the task, for
 ...
 * the asynchronous task are passed to this step. The result of the
 * computation must be returned by this step and will be passed back to the
 * last step. This step can also use {@link #publishProgress} to publish one
 * or more units of progress. These values are published on the UI thread, in
 * the {@link #onProgressUpdate} step.</li>
 ...
 * Invoking this method will cause subsequent calls to {@link #isCancelled()}
 * to return true. After invoking this method, {@link #onCancelled(Object)},
 * instead of {@link #onPostExecute(Object)} will be invoked after
 * {@link #doInBackground(Object[])} returns. To ensure that a task is
 * cancelled as quickly as possible, you should always check the return value
 * of {@link #isCancelled()} periodically from
 ...
 * </p>
 * <ul>
 * <li>Set member fields in the constructor or {@link #onPreExecute}, and
 * refer to them in {@link #doInBackground}.
...
public abstract class RLAsyncTask<Params, Progress, Result> {
     ...
     * {@link #publishProgress} to publish updates on the UI thread.
     * 
     ...
     * returns. Calling this method guarantees that
     * {@link #onPostExecute(Object)} is never invoked. After invoking this
     * method, you should check the value returned by {@link #isCancelled()}
     * periodically from {@link #doInBackground(Object[])} to finish the task
     ...
     * When first introduced, AsyncTasks were executed serially on a single
     * background thread. Starting with
     * {@link android.os.Build.VERSION_CODES#DONUT}, this was changed to a
     * pool of threads allowing multiple tasks to operate in parallel.
     * Starting {@link android.os.Build.VERSION_CODES#HONEYCOMB}, tasks are
     * back to being executed on a single thread to avoid common application
     * errors caused by parallel execution. If you truly want parallel
     * execution, you can use the {@link #executeOnExecutor} version of this
     * method with {@link #THREAD_POOL_EXECUTOR}; however, see commentary
     ...
     *             {@link RLAsyncTask.Status#RUNNING} or
     ...
     * AsyncTask, however you can also use your own {@link Executor} for
     * custom behavior.
     * <p>
     ...
     * can use this function with {@link #SERIAL_EXECUTOR}.
     * <p>
     * This method must be invoked on the UI thread.
     * 
    ...
    /**
     * Convenience version of {@link #execute(Object...)} for use with a
     * simple Runnable object. See {@link #execute(Object[])} for more
