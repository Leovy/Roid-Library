...
public class BitmapIndicator extends BaseIndicator
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (activeBitmap != null && inactiveBitmap != null)
        {
            ...
            for (int i = 0; i < getViewFlow().getViewsCount(); i++)
            {
                if (i == index)
                {
                    canvas.drawBitmap(activeBitmap, getPaddingLeft() + (getInnerMargin() + activeBitmap.getWidth())
                            * index, getPaddingTop(), mPaint);
                }
                else
                {
                    canvas.drawBitmap(inactiveBitmap, getPaddingLeft() + (getInnerMargin() + inactiveBitmap.getWidth())
                            * i, getPaddingTop(), mPaint);
                }
            }
        ...
        if (specMode != MeasureSpec.EXACTLY)
        {
            if (activeBitmap != null && inactiveBitmap != null)
            {
                ...
                width = (int) (getPaddingLeft() + getPaddingRight()
                        + (count * Math.max(activeBitmap.getWidth(), inactiveBitmap.getWidth())) + (count - 1)
                        * getInnerMargin() + 1);
            }
    ...
    protected int onMeasureHeight(int measureSpec)
    {
        ...
        if (specMode != MeasureSpec.EXACTLY)
        {
            if (activeBitmap != null && inactiveBitmap != null)
            {
                height = (int) (Math.max(activeBitmap.getHeight(), inactiveBitmap.getHeight()) + getPaddingTop()
                        + getPaddingBottom() + 1);
            }
            if (specMode == MeasureSpec.AT_MOST)
            {
