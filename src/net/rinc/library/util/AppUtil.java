...
import net.rinc.library.entity.AppInfo;
...
public class AppUtil {
	...
	public interface Listener{
		public void onReceived(ArrayList<AppInfo> list);
	}
	...
	public static void getInstalledAppList(final Context context, final Listener listener){
		new AsyncTask<Object,Object,ArrayList<AppInfo>>(){
			@Override
			...
			@Override
			public void onPostExecute(ArrayList<AppInfo> result){
	...
	public static void clearCache(Context context,String packageName){
		...
		if(pContext.getCacheDir()!=null){
			...
			if(root1.exists()&&SysUtil.requestRootPermission(root1.getPath())){
				for(String dir:root1.list()){
					FileUtil.deleteDir(new File(root1, dir));
				}
				UiUtil.toast(context, R.string.inner_cache_cleared);
	        }else{
	        	UiUtil.toast(pContext, R.string.request_root_failed);
	        }
		}
		String path2=null;
		if(SysUtil.isExternalStorageAvailable()
				&&(path2=SysUtil.getExternalCacheDir(pContext))!=null){
			File root2 = new File(path2);
			if(root2.exists()&&SysUtil.requestRootPermission(root2.getPath())){
				for(String dir:root2.list()){
					FileUtil.deleteDir(new File(root2, dir));
				}
				UiUtil.toast(context, R.string.external_cache_cleared);
	        }else{
	        	UiUtil.toast(pContext, R.string.request_root_failed);
	        }
		}
	...
	public static void clearData(Context context,String packageName){
		...
		if(pContext.getCacheDir()!=null&&pContext.getCacheDir().getParent()!=null){
			...
			if(root1.exists()&&SysUtil.requestRootPermission(path1)){
				for(String dir:root1.list()){
					FileUtil.deleteDir(new File(root1, dir));
				}
				UiUtil.toast(context, R.string.inner_data_cleared);
	        }else{
	        	UiUtil.toast(pContext, R.string.request_root_failed);
	        }
		}
		String path2=null;
		if(SysUtil.isExternalStorageAvailable()
				&&(path2=SysUtil.getExternalCacheDir(pContext))!=null&&new File(path2).getParent()!=null){
			File root2 = new File(new File(path2).getParent());
			if(root2.exists()&&SysUtil.requestRootPermission(path2)){
				for(String dir:root2.list()){
					FileUtil.deleteDir(new File(root2, dir));
				}
				UiUtil.toast(context, R.string.external_data_cleared);
	        }else{
	        	UiUtil.toast(pContext, R.string.request_root_failed);
	        }
		}
